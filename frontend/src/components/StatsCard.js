    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mb-2 md:mb-4">
                <h3 className="text-slate-400 font-medium text-base md:text-lg">{label}</h3>
                {icon && <div className="text-primary-400">{icon}</div>}
            </div>
            <p className="text-2xl md:text-4xl font-extrabold text-white tracking-tight">{value}</p>
        </motion.div>
    );
}