...
export default function PhotoModal({ photo, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-50 flex items-center justify-center p-4 md:p-8">
            <button
                onClick={onClose}
                className="absolute top-2 right-2 md:top-6 md:right-6 p-2 md:p-3 text-slate-400 hover:text-white rounded-full hover:bg-slate-800 transition-all z-20 shadow-lg"
            >
            ...
            >
                { }
                <div className="w-full md:w-3/4 bg-slate-950 flex items-center justify-center relative p-2 md:p-4 group">
                    <img
                        src={`http://localhost:5000/api/dashboard/media/${photo.filename}`}
                        alt={photo.filename}
                        className="max-w-full max-h-[50vh] md:max-h-[85vh] object-contain shadow-2xl drop-shadow-2xl"
                    />
                </div>
                {/* Details Section */}
                <div className="w-full md:w-1/4 bg-slate-800 flex flex-col p-4 md:p-6 overflow-y-auto border-t md:border-t-0 md:border-l border-slate-700/50">
                    <h3 className="text-lg md:text-xl font-bold text-white mb-4 md:mb-6">Photo Details</h3>
